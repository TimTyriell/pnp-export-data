...
# Citations are labelled with the episode id, so two members citing the same
# session already use the same marker — see pagemap.compose_body.
_LEAD_BODY = (
...
def test_compose_passes_a_lone_lead_through_untouched():
    """The 1:1 default is all but a handful of pages — it must not change."""

    assert pagemap.compose_body(_MEMBERS[:1], _BODIES) == _LEAD_BODY
...
def test_compose_nests_a_sub_under_its_own_heading():
    ...
    # One citation list; the shared session (P-22) appears once, and the
    # markers in both members' text still point at it — no renumbering needed.
    assert out.count("# Belege") == 1
    assert out.endswith("[P-08] Session A\n[P-22] Session B\n[P-34] Session C\n")
    assert out.count("[P-22]") == 3  # once per member's text, once in the list
...
def test_upload_only_targets_update_pages(tmp_path, monkeypatch):
    ...
    assert upload._update_titles() == {"Die Hexe", "Story Abschnitte"}
...
def test_overview_groups_by_season_newest_first():
    out = generate.render_story_overview(_OVERVIEW_PLAN)
    assert out.index("== Staffel 1 ==") < out.index("== Prolog ==")
...
def test_overview_proposal_needs_the_live_page(monkeypatch):
    monkeypatch.setattr(generate.config, "STORY_OVERVIEW_PAGE", "Story Abschnitte")
    events: list[dict] = []
    # Refuses to invent the page: a wrong title would create a second table.
    assert generate.build_overview_proposal(_OVERVIEW_PLAN, {}, events) == {}
    assert [e["event"] for e in events] == ["overview_skipped"]
